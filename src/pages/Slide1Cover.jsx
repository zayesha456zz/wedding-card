...
function Slide1Cover({ onOpen }) {
  ...
  return (
    <div className="relative font-serif select-none flex items-center justify-center overflow-hidden bg-[#faf6ee] w-full h-screen">
      ...
      {showNextPage && (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10 bg-black/10 animate-fadeIn px-4">
          ...
          {/* Petals layer - z-index badha kar 50 kar diya hai */}
          ...
          {/* Card layer - z-index 30 rakha hai taake petals iske upar rahen */}
          <div className="p-6 md:p-10 border border-[#dfb15b]/40 bg-white/90 backdrop-blur-sm shadow-2xl relative z-30 w-full max-w-sm md:max-w-lg rounded-3xl">
            <p className="text-[#b8862d] tracking-[0.4em] text-[8px] md:text-[9px] uppercase mb-4 font-sans">بِسْمِ اللهِ الرَّحْمٰنِ الرَّحِيْمِ</p>
            <h2 className="text-[9px] md:text-[10px] uppercase tracking-[0.5em] text-neutral-600 mb-8 font-sans">The Wedding Celebration Of</h2>
            
            <div className="flex flex-row items-center justify-center gap-4 relative z-40">
              <div className="flex flex-col items-center flex-1">
                <div className="w-20 h-28 md:w-28 md:h-36 border-[3px] border-[#dfb15b] p-1 bg-neutral-200 rounded-t-full">
                  <img src="/bride.jpeg" alt="Kinza" className="w-full h-full object-cover rounded-t-full" />
                </div>
                <h1 className="mt-3 text-base md:text-lg font-bold text-[#8b5c1a]">Kinza</h1>
              </div>
              <div className="text-xl md:text-2xl font-bold text-[#dfb15b] pb-8">&</div>
              <div className="flex flex-col items-center flex-1">
                <div className="w-20 h-28 md:w-28 md:h-36 border-[3px] border-[#dfb15b] p-1 bg-neutral-200 rounded-t-full">
                  <img src="/groom.jpeg" alt="Umar" className="w-full h-full object-cover rounded-t-full" />
                </div>
                <h1 className="mt-3 text-base md:text-lg font-bold text-[#8b5c1a]">Umar</h1>
              </div>
            </div>

            <div className="w-32 h-px bg-[#dfb15b]/40 mx-auto mt-6 mb-6" />
            <p className="text-xs md:text-sm text-neutral-700 font-serif italic leading-relaxed tracking-wide px-2">
      ...
      {!showNextPage && (
        <div className="absolute inset-0 z-40 flex items-center justify-center w-full h-full overflow-hidden">
          ...
          {!isOpened && (
            <button onClick={handleOpen} className="absolute z-50 px-10 py-4 bg-[#dfb15b] text-white font-bold tracking-[0.2em] uppercase shadow-xl hover:bg-[#b8862d] transition-all rounded-full bottom-[20%]">
              OPEN
            </button>
          )}
        </div>
